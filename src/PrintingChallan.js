...
const pickDefaultVisibleColumns = (allCols = []) => {
  const groups = [
    ...
    // Add priority to default columns
    { id: 'priority', pats: ['priority', 'prio', 'importance'] },
  ];
  ...
  if (res.length < 5) return allCols.slice(0, Math.min(8, allCols.length));
  return res;
};
...
const handleCreateChallan = async (row, extras = {}) => {
  ...
  try {
    ...
    doc.text("PRINTING CHALLAN", PAGE.w / 2, 62, { align: "center" });
...
const generateChallanPdfLocal = async (row, entry) => {
  ...
  const party       = first(['Party Name','Party','Buyer','Customer']);
  const style       = first(['Style','Style No','Style Name']);
  const Print         = first(['Printing','Design','Design Name']);
  const printDet      = first(['Printing Details','Printing Details']);
  const fabric      = first(['Fabric','Fabric Type']);
  const size        = first(['Size','Sizes']);
  ...
  const drawOneCopy = (doc, leftOffset, SCALE, copyLabel, rowsForHalf) => {
    ...
    doc.text('PRINTING CHALLAN', leftX, baselineY);
    ...
    writeCell(3, 0, 'Printing',          Print);
    // Row 2
    writeCell(0, 1, 'Size',         size);
    writeCell(1, 1, 'Fabric',       fabric);
    writeCell(2, 1, 'Printing Details',  printDet);
...
autoTable(doc, {
  ...
  for (const SCALE of TRY_SCALES) {
    ...
    // left half
    drawOneCopy(doc, 0, SCALE, 'PRINTING HEAD COPY', rows10);
          ...
          {embstatus ? (
            <span
              style={{
                ...
                fontWeight:700,
                letterSpacing:'.2px',
                background: embstatus === 'Complete Printing'
                  ? 'rgba(34,197,94,.15)'
                  : 'rgba(245,158,11,.15)',
                border: '1px solid ' + (embstatus === 'Complete Printing'
                  ? 'rgba(34,197,94,.35)'
                  : 'rgba(245,158,11,.35)'),
                color: embstatus === 'Complete Printing' ? '#22c55e' : '#f59e0b',
              }}
              title={embstatus}
            >
              {embstatus}
