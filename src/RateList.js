...
async function buildAndDownloadJPEG_Receipt_A4({
  ...
  // Signature section (4 boxes: Pintu, Mohit Sir, Submitter, Any Change)
 ...
 const fetchRates = useCallback(async () => {
  ...
  const getRate = (attr, opt) => {
    const match = rows.find(
      (r) => r.Category === category && r.Attribute === attr && r.Option === opt
    );
    return match?.Rate ?? 0;
  };
  ...
  // Quantity lookup using canonical key (supports Hindi/English)
  const qtyForKey = (key) => {
    switch (key) {
      case "cut":
  ...
  const billRows = useMemo(() => {
    ...
    Object.entries(normalizedForm).forEach(([attr, opts]) => {
      ...
      const qtyVal = isQtyKey(key) ? qtyForKey(key) : 1;

      opts.forEach((opt) => {
        const rate = getRate(attr, opt) || 0;
        const amount = isQtyKey(key) ? rate * qtyVal : rate;
        lines.push({ attr, opt, rate, qty: qtyVal, amount });
      });
    });
    return lines.sort(
      (a, b) => a.attr.localeCompare(b.attr) || a.opt.localeCompare(b.opt)
    );
  }, [normalizedForm, qty.Cut, qty.Label, qty.Bone, qty.Gulla, qty.Teera, rows, category]);
...
// -   const res = await fetch(GAS_WEB_APP_URL, {
   // Start the save request (but don't await yet)
...
// -   }
   ...
   // In parallel, start generating the JPEG
   ...
   // Ensure JPEG has finished (if it hasn't already)
   await jpegPromise;
      ...
      <div className="rc-wrap">
        ...
        >
          <div className="rc-title">
            ...
            <div>
              ...
              <p>Pick a category and build up the garment&apos;s stitching rate</p>
            </div>
          </div>
        ...
        <div className="rc-layout">
          ...
          <div className="rc-card rc-main-card">
            ...
            <div className="rc-field rc-field-enhanced">
              ...
              <div className="rc-select-wrap">
                ...
                <span className="rc-chev" aria-hidden>▾</span>
              </div>
              <p className="rc-hint">
                Multi-select supported via <b>checkbox dropdowns</b> for most
                attributes.
              </p>
            </div>
              ...
              <div className="rc-grid">
                {Object.keys(attributes).map((attr) => {
                  ...
                  const selArr =
                    ...
                    [];
                  const key = toCanonicalAttr(attr);
                  const isSingle = isQtyKey(key);

                  // Label strings for qty field
                  const qtyLabel =
                    key === "cut"
                      ? "How many cuts?"
                      : key === "label"
                        ...
                        <>
                          <SingleSelectDropdown
                            ...
                            onChange={(v) => {
                              ...
                              // Auto set qty to 1 when selecting any qty-based attr if currently 0 (except Cut which is already 1)
                              if (v) {
                          ...
                          {selArr[0] && (
                            <div className="rc-qty-row">
                              ...
                              <span className="rc-hint">
                                Per-{key} rate: <b>{formatINR(getRate(attr, selArr[0]))}</b>
                              </span>
                            </div>
                          )}
                        </>
        ...
        <footer className="rc-foot">
          ...
          <span>
            Tip: Use the dropdowns; most attributes support multiple selection via
            checkboxes.
          </span>
        </footer>
      </div>
